...
class _QuestionScreenState extends State<QuestionScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _checkIfLevelAlreadyCompleted();
    });
  }

  Future<void> _checkIfLevelAlreadyCompleted() async {
    final levelKey = 'level${widget.level}Completed';
    final isCompleted = await storage.read(key: levelKey);
    if (isCompleted == 'true') {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 202) {
        if (data['success'] == true) {
          final questions = data['data']['ques'];

          if (questions != null && questions.isNotEmpty) {
            setState(() {
              _currentQuestion = questions[0];
              _score = data['data']['score'] ?? 0;
              _questionNumber = questions[0]['id'] ?? 1;
            });
          } else if (data['data']['message'] == 'Level is finished') {
            final levelKey = 'level${widget.level}Completed';
            await storage.write(key: levelKey, value: 'true');
            ...
            _showErrorDialog(data['message'] ?? 'No question found.');
          }
        } else {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 202) {
        if (data['success'] == true) {
          ...
          _showErrorDialog(data['message'] ?? 'Incorrect answer.');
        }
      } else {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 202) {
        ...
        if (data['success'] == true &&
          ...
          setState(() {
            _isHintVisible = true;
            if (!_isHintUsed) _isHintUsed = true;
                          ...
                          child: Row(
                            children: [
                              Icon(Icons.lightbulb_outline,
                                  color: Colors.yellow, size: scale(20)),
